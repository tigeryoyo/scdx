...
function initShowPage(currenPage) {
	...
	$.ajax({
		type : "post",
		url : "/rankWeight/selectWeightCount",
		dataType : "json",
		beforeSend : function() {
			begin();
			},
...
function weightChange(value1, value2, value3) {
	...
	setCookie("rankWeightInfor", JSON.stringify(json));
	jumpto("rank-weight-change");
}
...
function weightInforSearch(page) {
	...
	$
		.ajax({
			type : "post",
			url : "/rankWeight/selectByCondition",
			data : {
				name : $("#name").val(),
				weight : $("#weight").val(),
				start : (parseInt(10 * page - 10)),
...
function weightInforAdd() {
	jumpto("rank-weight-add");
}
function addWeight() {
	$.ajax({
		type : "post",
		url : "/rankWeight/insertWeight",
		data : {
			name : $("#add_type").val(),
...
function weightInforChange() {
	var weight = JSON.parse(getCookie("rankWeightInfor"));
	$.ajax({
		type : "post",
		url : "/rankWeight/updateWeight",
		data : {
			id : weight.weightId,
